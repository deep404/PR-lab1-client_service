...
import random
import threading
from addons import json_log
from order.Order import ClientOrderDto
from order.Order import OrderDto
...
class Client:
    ...
    def generate_order(self):
        ...
        response = requests.get(f'http://{FO_HOST}:{FO_PORT}/menu')
        data = response.json()
    ...
    def client_wait_single_order(self, fo_response, single_order):
        ...
        while not is_ready:
            ...
            response = requests.get(f'http://{DH_HOST}:{single_order["restaurant_address"]}/v2/order/{fo_response["order_id"]}')
            json_ = response.json()
            ...
            if json_['is_ready']:
                ...
                res = requests.post(f'http://{FO_HOST}:{FO_PORT}/rating', json=req)
